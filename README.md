...
| **A commit inside a script file — `INC-0033`, STILL OPEN** | `bash ./do-commit.sh` is not text containing `git commit`, so the trigger never fires and **the stamp is never checked**. Measured against a repo with no stamp at all: `git commit` → exit 2, `git -C PATH commit` → exit 2, `sh -c "git commit"` → exit 2, `bash ./do-commit.sh` → **exit 0**. Worse than the five above, because **this was the workflow this project documented**: the gate blocked any Bash call merely *mentioning* `git commit`, so wrapping it in a script became routine — and the workaround for the false positive disabled the true positive. **0.6.1 removed the motive, not the capability**: the trigger now requires `git` at a command position, so writing *about* a commit is no longer refused. `bash ./x.sh` is still allowed. The incident stays open, and a control in `test-verify-gate.sh` asserts this row is still true. |
